...
GLint timeLoc[SHADERS_NUM];
GLint demoLoc[SHADERS_NUM];
GLint paletteLoc[SHADERS_NUM];
GLint colorVolLoc[SHADERS_NUM];

GLuint palette_tex;
GLuint volume_tex;

// Demo modes for GLSL 330 builtin showcase
enum {
	DEMO_REFLECT = 0,
	DEMO_BITFIELD = 1,
	DEMO_VOLUME = 2,
...
static void checkShaderCompile(GLuint shader, const char *label) {
	...
	char *log = (char *)malloc(log_len > 1 ? log_len : 512);
	glGetShaderInfoLog(shader, log_len, NULL, log);
	fatal_error("%s compile error:\n%s", label, log);
}

static void checkProgramLink(GLuint program, const char *label) {
	GLint status = GL_FALSE;
	...
	fatal_error("%s link error:\n%s", label, log);
}

static void createPaletteTexture(void) {
	const int w = 256;
	const int h = 8;
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			...
			data[i + 0] = (uint8_t)(127.0f + 127.0f * sinf(t * 6.28318f + y * 0.4f));
			data[i + 1] = (uint8_t)(127.0f + 127.0f * sinf(t * 6.28318f + 2.094f));
			data[i + 2] = (uint8_t)(127.0f + 127.0f * sinf(t * 6.28318f + 4.188f));
		}
	}
...
static void createVolumeAtlas(void) {
	const int slice = 16;
	const int depth = 8;
	const int w = slice * depth;
	const int h = slice;
	uint8_t *data = (uint8_t *)malloc(w * h * 3);
	for (int z = 0; z < depth; z++) {
		for (int y = 0; y < slice; y++) {
			for (int x = 0; x < slice; x++) {
				int px = z * slice + x;
				int i = (y * w + px) * 3;
				data[i + 0] = (uint8_t)((x * 255) / (slice - 1));
				data[i + 1] = (uint8_t)((y * 255) / (slice - 1));
				data[i + 2] = (uint8_t)((z * 255) / (depth - 1));
			}
		}
	}
...
void loadShader(const char *name, int type) {
	...
	checkProgramLink(programs[type], name);
	...
	if (type == MIRROR) {
		...
		paletteLoc[type] = glGetUniformLocation(programs[type], "palette");
		colorVolLoc[type] = glGetUniformLocation(programs[type], "colorVol");
		demoLoc[type] = glGetUniformLocation(programs[type], "uDemo");
	}
	...
	timeLoc[type] = glGetUniformLocation(programs[type], "uTime");
...
int main() {
	...
	// Texture units: 0 = cubemap, 1 = 2D palette, 2 = 3D volume atlas
	glUniform1i(glGetUniformLocation(programs[SKYBOX], "skybox"), 0);
	glUniform1i(glGetUniformLocation(programs[MIRROR], "skybox"), 0);
	glUniform1i(paletteLoc[MIRROR], 1);
	glUniform1i(colorVolLoc[MIRROR], 2);
	...
	for (;;){
		...
		// Cycle GLSL 330 demo mode (bitfield / volume / all)
		if (CHECK_BTN(SCE_CTRL_SELECT)) {
			demo_mode = (demo_mode + 1) % DEMO_NUM;
		}
		...
		glUniform1f(timeLoc[MIRROR], currentFrame);
		glUniform1i(demoLoc[MIRROR], demo_mode);
		
		// View matrix: camera position, view direction, camera "up" vector
		glm::mat4 view = glm::lookAt(camera_pos, camera_pos + camera_front, camera_up);
		glUniformMatrix4fv(viewMatrixLoc[MIRROR], 1, GL_FALSE, glm::value_ptr(view));
		
		// Binding showcase textures
		...
		glUniform1f(timeLoc[SKYBOX], currentFrame);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_CUBE_MAP, skybox_tex);
