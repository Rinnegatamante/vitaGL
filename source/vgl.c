...
int check_Sharpscale(void) {
	int search_unk[2];
	return _vshKernelSearchModuleByName("Sharpscale", search_unk);
}
...
GLboolean vglInitWithCustomSizes(int pool_size, int width, int height, int ram_pool_size, int cdram_pool_size, int phycont_pool_size, SceGxmMultisampleMode msaa) {
	...
	if (!(sceKernelIsPSVitaTV() && check_Sharpscale() >= 0) && !(width <= 960 && height <= 544) || (width > 1920 || height > 1088)) {
		DISPLAY_WIDTH = 960;
		DISPLAY_HEIGHT = 544;
